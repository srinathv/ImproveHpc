 ...
 * - NBodyLocals_AOS: Similarly optimized version but operating on arrays-of-structs 
...
protected:
    ...
    void check (FTYPE a, FTYPE b, int i, const char* msg, int& errs, FloatError&);
...
int NBody<float>::id = Workload::registry(NBody<float>::name, NBody<float>::create);

/* does not work with default vectorlength due to compiler bug, try vectorlength(16)
const char* NBody<double>::name ()        {return "nbody_d";}
int NBody<double>::id = Workload::registry(NBody<double>::name, NBody<double>::create);
*/

const char* NBodyLocals<float>::name ()        {return "NBodyLocals";}
int NBodyLocals<float>::id = Workload::registry(NBodyLocals<float>::name, NBodyLocals<float>::create);

const char* NBodyLocals_AOS<float>::name ()        {return "NBodyLocals_AOS";}
int NBodyLocals_AOS<float>::id = Workload::registry(NBodyLocals_AOS<float>::name, NBodyLocals_AOS<float>::create);
...
template <class FTYPE>
void NBody<FTYPE>::check (FTYPE a, FTYPE b, int i, const char* msg, int& errs, FloatError& err)
{
    if (err.errorf(a, b))
    {
        if (++errs < 10)
            cout << " body# " << i << ", " << msg << ": " << a << " != " << b << endl;
    }
}
...
bool NBody<FTYPE>::validate ()
{
    ...
    if (errs != 0)
        cout << "total errors: " << errs
              << " (max error for r=" << err_r.max_error << ", v=" << err_v.max_error << ")"
              << endl;
...
void NBody<FTYPE>::execute_serial (RV<FTYPE>* out)
{
    ...
    FTYPE * init_pos_x = init->rx, * init_pos_y = init->ry, * init_pos_z = init->rz;
    FTYPE * init_vec_x = init->vx, * init_vec_y = init->vy, * init_vec_z = init->vz;

    FTYPE * test_pos_x = out->rx, * test_pos_y = out->ry, * test_pos_z = out->rz;
    FTYPE * test_vec_x = out->vx, * test_vec_y = out->vy, * test_vec_z = out->vz;
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        test_pos_x[i] = init_pos_x[i] + init_vec_x[i]*time_step + ax*time_step*time_step/2.0f;
        test_pos_y[i] = init_pos_y[i] + init_vec_y[i]*time_step + ay*time_step*time_step/2.0f;
        test_pos_z[i] = init_pos_z[i] + init_vec_z[i]*time_step + az*time_step*time_step/2.0f;
    }
}
...
__declspec(vector(linear(i), \
    uniform(body_count, mass, init_pos_x, init_pos_y, init_pos_z, init_vec_x, init_vec_y, init_vec_z),\
    uniform(test_pos_x, test_pos_y, test_pos_z, test_vec_x, test_vec_y, test_vec_z),\
    uniform(time_step, epsilon)))
#endif // __GFX__
static void calculate_body (int i, int body_count, 
                            FTYPE * mass,
                            FTYPE * init_pos_x, FTYPE * init_pos_y, FTYPE * init_pos_z, 
                            FTYPE * init_vec_x, FTYPE * init_vec_y, FTYPE * init_vec_z,
                            FTYPE * test_pos_x, FTYPE * test_pos_y, FTYPE * test_pos_z,
                            FTYPE * test_vec_x, FTYPE * test_vec_y, FTYPE * test_vec_z,
...
#pragma offload target(gfx) if (do_offload > 0)\
    pin(mass, init_pos_x, init_pos_y, init_pos_z, init_vec_x, init_vec_y, init_vec_z, \
        test_pos_x, test_pos_y, test_pos_z, test_vec_x, test_vec_y, test_vec_z: length(body_count)) 
...
void NBodyLocals<FTYPE>::execute_part(int do_offload, int off, int n)
{
    ...
    _Cilk_for (int i = off; i < off + n; i+=TILE_i)
    {
        ...
        for (int k = 0; k<TILE_i; k++)
        {
            ...
            test_pos_x[i+k] = rx[k] + vx*time_step + ax[k]*time_step*time_step/2.0f;
            test_pos_y[i+k] = ry[k] + vy*time_step + ay[k]*time_step*time_step/2.0f;
            test_pos_z[i+k] = rz[k] + vz*time_step + az[k]*time_step*time_step/2.0f;
        }
    }
}
...
void NBodyLocals_AOS<FTYPE>::execute_part(int do_offload, int off, int n)
{
    ...
    _Cilk_for (int i = off; i < off + n; i += TILE_i)
    {
            ...
            for (int j_ = 0; j_ < TILE_j; j_++)
            {
                ...
                for (int k=0; k<TILE_i; k++)
                {
                    FTYPE dx = ix[j_] - rx[k],
                          dy = iy[j_] - ry[k],
                          dz = iz[j_] - rz[k];
                    FTYPE distanceInv = 1.0f / sqrtf(dx*dx + dy*dy + dz*dz + epsilon);
        ...
        for (int k = 0; k<TILE_i; k++)
        {
            ...
            out_pos4[i+k].x = rx[k] + vx * time_step + ax[k] * time_step * time_step * 0.5f;
            out_pos4[i+k].y = ry[k] + vy * time_step + ay[k] * time_step * time_step * 0.5f;
            out_pos4[i+k].z = rz[k] + vz * time_step + az[k] * time_step * time_step * 0.5f;
        }
    }
}
