 ...
 *    loops which substantially increases the compile time. So, for convenience, 
 *    there are COMPILE_*_VERSION macros enabling or disabling
...
int Matmult<float>::id = Workload::registry(Matmult<float>::name, Matmult<float>::create);

const char* MatmultLocalsAN<float>::name ()        {return "MatmultLocalsAN";}
int MatmultLocalsAN<float>::id = Workload::registry(MatmultLocalsAN<float>::name, MatmultLocalsAN<float>::create);

const char* Matmult<double>::name ()        {return "Matmult_d";}
int Matmult<double>::id = Workload::registry(Matmult<double>::name, Matmult<double>::create);

const char* MatmultLocalsAN<double>::name ()        {return "MatmultLocalsAN_d";}
int MatmultLocalsAN<double>::id = Workload::registry(MatmultLocalsAN<double>::name, MatmultLocalsAN<double>::create);
...
bool Matmult<FTYPE>::open ()
{
    ...
    m_matC = (FTYPE*)w_malloc_check(m_matCsize*sizeof(FTYPE)); memset(m_matC, 0, m_matCsize*sizeof(FTYPE));
    m_matGold = 0;
...
bool MatmultLocalsAN<FTYPE>::open ()
{
    ...
    if (this->m_height % TILE_m != 0){
        cout << "For tiled implementation height must be divisible by " << TILE_m <<endl;
        return false;
    }
    if (this->m_width % TILE_n != 0){
        cout << "For tiled implementation width must be divisible by " << TILE_n <<endl;
        return false;
    }
    if (this->m_common % TILE_k != 0){
        cout << "For tiled implementation width must be divisible by " << TILE_k <<endl;
        return false;
    }
...
#ifdef MKL
//To compile:
//  (1) Run setup script <compiler installation dir>\mkl\bin\ia32\mklvars_ia32.bat
...
void Matmult<FTYPE>::execute_part(int do_offload, int off, int m)
{
    ...
    int common = this->m_common, width  = this->m_width, height = this->m_height;
    #pragma offload target(gfx) if (do_offload > 0) \
        pin(matA: length(m_matAsize)) pin(matB: length(m_matBsize)) pin(matC: length(m_matCsize))
...
void MatmultLocalsAN<FTYPE>::execute_part(int do_offload, int off, int m)
{
    ...
    #pragma offload target(gfx) if (do_offload) \
        pin(A: length(this->m_matAsize)) pin(B: length(this->m_matBsize)) pin(C: length(this->m_matCsize))
