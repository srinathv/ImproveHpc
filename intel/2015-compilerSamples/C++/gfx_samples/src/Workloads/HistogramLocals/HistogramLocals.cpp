 ...
 *  - For array or 4-component pixels (4 bytes per pixel), separate for each components 
 *
...
bool HistogramLocals::open () 
{
    ...
    if ( (m_szMatrix % m_szGlobalWorkStep) != 0){
        cout << "szMatrix must be divisible by szGlobalWorkStep in this simple example implementation" <<endl;
        return false;
    }
    m_szMatrix *= 4; // input value in 4-byte elements, convert to byte count

    m_inputArray = (unsigned char*)w_malloc_check(m_szMatrix);
    m_tmpArray = (unsigned int *)w_malloc_check(m_szHistogram * m_szGlobalWorkStep * sizeof(unsigned int));
    m_outputArray = (unsigned int *)w_malloc_check(m_szHistogram * sizeof(unsigned int));
...
{
    unsigned int * referenceArray = (unsigned int *)w_malloc_check(m_szHistogram * sizeof(unsigned int));
        ...
        if (m_outputArray[i] != referenceArray[i]) {
            if (errorCount < maxErrorCount){
                cout << "INCORRECT result at index " << i << ", Result="
                    << m_outputArray[i] << ", Reference result=" << referenceArray[i] << endl;
            }
            errorCount++;
        }
    }
...
bool HistogramLocals::execute_offload(int do_offload) 
{
    ...
    #pragma offload target(gfx) if (do_offload > 0) \
        pin(inputArray: length(szMatrix)) pin(tmpArray: length(256*szGlobalWorkStep)) 
    _Cilk_for (int i = 0; i < szGlobalWorkStep; i++) {
        int out[256];
        out[:] = 0;
        for (unsigned int m = 0; m < szPerThread; m+=256) {
            int ip[256];
...
bool HistogramLocals4::execute_offload(int do_offload) 
{
    ...
    #pragma offload target(gfx) if (do_offload > 0) \
        pin(inputArray: length(szMatrix)) pin(tmpArray: length(1024*szGlobalWorkStep)) 
    _Cilk_for (int i = 0; i < szGlobalWorkStep; i++) {
        int out[256];
        #define BS 256
        for (int m = 0; m < szPerThread; m+=1024) {
