...
public:
	...
	bool execute_offload (int do_offload)	{return execute_offload(image_dst, do_offload);}
...
//  setup filter
    ...
    filter = (float*)w_malloc_check((filter_width * filter_height) * sizeof(float));
...
//  setup source image
    ...
    image_src_total_size = image_src_row_size * (image_height + padding_vert * 2);
    ...
    for (int y = 0; y < image_height; ++y){
        for (int x = 0; x < image_width; ++x){
            image_src[image_src_offset + y * image_src_row_size + x] = rand() % 256;
        }
    }    
    image_ref = 0;
	int sz;
    image_dst = (float*)w_malloc_check(sz = image_width * image_height * sizeof(int)); memset(image_dst, 0, sz);
    return true;
}
...
bool ConvolutionLocalsAN::open ()
{
    ...
    if (filter_width > max_filter_size || filter_height > max_filter_size){
		cout << "filter size too big (max = " << max_filter_size << ")" << endl;
		return false;
	}
...
{
    if (image_ref == 0){
        image_ref = (float*)w_malloc_check(image_width * image_height * sizeof(int));
        ...
        for (int ix = 0; ix < image_width; ++ix){
            ...
            for (int fy = 0; fy < filter_height; ++fy){
                for (int fx = 0; fx < filter_width; ++fx){
                    ...
                    sum += filter[fy * filter_width + fx] * (float)image_src[image_src_offset + ify * image_src_row_size + ifx];
                }
            }
...
__declspec(vector( linear(ix),  \
    uniform(iy, filter_width, filter_height, filter_addr, src_rowsz, src_addr, image_width, dst_addr) \
    ))
...
bool ConvolutionLocalsAN::execute_cpu ()
{
    ...
    _Cilk_for (int i = 0; i < image_height; i+= tile_height){
        ...
        for (int j = 0; j < image_width; j+= tile_width){
            ...
            int   (* src_addr)[image_src_row_size]   = (int   (*)[])(image_src + image_src_offset); 
            const int src_tile_v_offset = filter_height / 2;
            const int src_tile_height = tile_height + src_tile_v_offset * 2;
            const int src_tile_h_offset = 8; // assuming 8 is >= than max_filter_width/2
            ...
            src_tile[0:src_tile_height][:] = src_addr[i - src_tile_v_offset:src_tile_height][j - src_tile_h_offset:src_tile_width];
            ...
            for (int fy = 0; fy < filter_height; fy++){
                ...
                for (int fx = 0; fx < filter_width; fx++){
                    ...
                    #pragma unroll
                    dst_tile[:][:] += fv * src_tile[src_tile_y:tile_height][src_tile_x:tile_width];
                }
            }

            #pragma unroll
            for (int ii = 0; ii < tile_height; ii++){
                dst_addr[(i + ii) * image_width + j:tile_width] = dst_tile[ii][:];
            }
        }
    }
